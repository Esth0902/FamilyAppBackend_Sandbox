...
class TaskRecurrenceService
{
     ...
     */
    public function normalizeTemplateRecurrenceAndDays(string $recurrence, mixed $rawRecurrenceDays): array
    {
        $normalizedRecurrence = in_array($recurrence, ['daily', 'weekly', 'monthly', 'once'], true)
            ? $recurrence
            : 'weekly';
        $days = Normalization::recurrenceDays($rawRecurrenceDays);
    ...
    public function resolveTemplateStartDate(
        string $recurrence,
        mixed $rawStartDate,
        ?string $templateStartDate = null,
        ?CarbonInterface $templateCreatedAt = null,
        ...
        if (is_string($templateStartDate) && trim($templateStartDate) !== '') {
            return Carbon::createFromFormat('Y-m-d', trim($templateStartDate))->startOfDay()->toDateString();
        }

        if ($templateCreatedAt instanceof CarbonInterface) {
            return $templateCreatedAt->copy()->startOfDay()->toDateString();
        }

        return ($referenceDate ?? now())->copy()->startOfDay()->toDateString();
    }

    public function resolveTemplateEndDate(string $recurrence, mixed $rawEndDate, ?string $startDate): ?string
    {
        if ($recurrence === 'once') {
            return null;
        }
    ...
    public function resolveTemplateStartDateValue(
        string $recurrence,
        ?string $templateStartDate,
        ?CarbonInterface $templateCreatedAt,
    ): ?string {
        if (is_string($templateStartDate) && trim($templateStartDate) !== '') {
            return Carbon::createFromFormat('Y-m-d', trim($templateStartDate))->startOfDay()->toDateString();
        }

        if ($recurrence !== 'once' && $templateCreatedAt instanceof CarbonInterface) {
            return $templateCreatedAt->copy()->startOfDay()->toDateString();
        }
    ...
    /**
     * @param  array<string, mixed>  $tasksConfig
     * @return array{enabled:bool,change_day:int,home_week_start:string|null}
     */
    public function resolveAlternatingCustodySettings(array $tasksConfig, ?CarbonInterface $referenceDate = null): array
    {
        ...
        return [
            ...
            'home_week_start' => $this->resolveCustodyHomeWeekStart(
                $enabled,
    ...
    public function resolveInterHouseholdWeekStart(
        bool $isEnabled,
        mixed $rawWeekStart,
        int $weekStartDay,
        ?CarbonInterface $referenceDate = null,
    ...
    public function resolveCustodyHomeWeekStart(
        bool $isEnabled,
        mixed $rawDate,
        int $changeDay,
        ?CarbonInterface $referenceDate = null,
    ...
    public function templateAppliesToDate(TaskTemplate $template, Carbon $date, int $interHouseholdWeekStartDay): bool
    {
     ...
     */
    public function resolveAssigneeIds(TaskTemplate $template, Collection $members, Carbon $date): array
    {
        $memberIds = $members
     ...
     */
    public function resolvePrimaryAssigneeId(array $assigneeIds): int
    {
     ...
     * @param  Collection<int, array{id:int,name:string,role:string}>  $members
     */
    public function isAlternatingCustodyEnabledForChildAssignee(
    ...
    public function isDateInAlternatingCustodyHomeWeek(Carbon $date, array $alternatingCustody): bool
    {
        if (!(bool) ($alternatingCustody['enabled'] ?? false)) {
            return true;
        }
    ...
    public function startOfCustomWeek(Carbon $date, int $startDayIso): Carbon
    {
